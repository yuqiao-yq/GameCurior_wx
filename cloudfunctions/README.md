...
# 冒烟：Switch 3 款
./scripts/sync-igdb.sh --platforms=130 --limit=3

# 全量：5 大主机 × 30 款（去重后 ~75 条，~10 秒）
./scripts/sync-igdb.sh
```

预期最后看到：`{ source: 'igdb', mode: 'batch', total: N, inserted: N, updated: 0, failed: 0 }`

**手动两步**（脚本内部等价于）：
```bash
cd scripts
node sync-igdb-local.js --platforms=130 --limit=3    # 1. 拉 IGDB 数据写 .igdb-batch.json
tcb fn invoke syncFromIGDB \                          # 2. 喂给云函数入库
  --params "$(cat .igdb-batch.json)" \
  -e cloud1-8g8jrsgc94538121
```

Token 缓存在 `scripts/.igdb-token.json`（`.gitignore` 已排除），~60 天有效自动续期。
